...
<?php
    if (isset($_POST["submit"]))
    {
        $backlink = "<a href=\"reg_admin.php?back=1\">".$lang["back"]."</a>";
            ...
            (isset($password_retry)))
        {
            ...
            $result = mysql_query("SELECT * FROM admins WHERE login='".$login."'", $db);
            ...
            if ($num == 0) //якщо логін не існує то створюємо ...
            {
                ...
                if ($password == $password_retry)
                {
                    ...
                    $result = mysql_query("INSERT INTO admins (login, FirstName, LastName, day, month, year, sex, password)
                                           VALUES ('".$login."', '".$FirstName."', '".$LastName."', '".$day."', '".$month."', '".$year."', '".$sex."', '".$password."');", $db) or die($myrow["title_".$language]);
                    if ($result == "true")
                    {
